...
def replay(fixture: dict, *, verbose: bool = True) -> bool:
    ...
    def _one_turn(msg, hist, exp):
        """Run a single decide() turn under the deterministic patches and
        check it against `exp`. Returns (passed, reply_text)."""
        stages = {}
        ...
        with patch("services.action_executor.requests.request",
            ...
            result = engine.decide(msg, history=hist, context=context)
        ...
        if exp.get("reply_contains"):
            checks.append(("reply contains " + repr(exp["reply_contains"]),
                           all(s in reply for s in exp["reply_contains"]), True))
        if exp.get("reply_not_contains"):
            checks.append(("reply NOT contains " + repr(exp["reply_not_contains"]),
                           all(s not in reply for s in exp["reply_not_contains"]), True))
        ...
        _p("PRIVATE STATE:     ", _fmt(dev.get("private_state_inquiry")))
    ...
    if seq:
        ...
        for i, step in enumerate(seq, 1):
            _p(f"===== SEQUENCE STEP {i}/{len(seq)} =====")
            ok, reply = _one_turn(step["message"], hist, step.get("expected") or {})
            all_passed = all_passed and ok
            hist = hist + [{"role": "user", "content": step["message"]},
                           {"role": "assistant", "content": reply}]
        return all_passed

    ok, _ = _one_turn(message, history, fx.get("expected") or {})
    return ok
