...
- **Test suite**: ~85 `unittest` files covering nearly every service/rag module; run via `python -m unittest discover -s tests -q`.
...
- **Semantic Business Layer's exportable metadata** is computed and displayed in the Admin UI but is **not yet consumed** by anything (Playground, Decision Engine, RAG, Prompt Builder, multi-ERP orchestration) — it was deliberately built as a separate, non-wired-in layer. See Next Steps.
