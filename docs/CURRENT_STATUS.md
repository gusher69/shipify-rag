...
- **LangGraph agent + Langfuse observability** (`services/agent/`, `services/observability/`, 2026-09-14): a bounded 13-node `StateGraph` that normalises, resolves the turn ONCE, plans, and EXECUTES through the existing DecisionEngine (no second decision engine). `LANGGRAPH_MODE` = `off` / `shadow` (default; telemetry only) / `owner_test` / `production` (graph primary for REAL_LINE, OWNER_TEST and ADMIN_AUTO; the DecisionEngine remains the automatic technical/safety fallback — never a semantic one). Langfuse is degrade-safe and privacy-masked. See `reports/agent_langgraph_mvp.md`.
- **Thai human-language input layer** (`services/language/`, 2026-09-15): PyThaiNLP + RapidFuzz normalisation of noisy Thai (typos, spacing, repeated marks, informal particles, keyboard layout) against a BOUNDED vocabulary, with identifier protection and `raw_text` always preserved. Runs as the graph's first node (`normalize_language`); MEDIUM candidates are decided by the central interpreter (semantic recovery), never applied blindly. Lab + gates: `tests/language_lab/`, `tests/test_language_intelligence_gate.py`, `reports/thai_language_intelligence.md`.
